...
/* ====================== SNAKE GAME PHASE 2 (full, chunked, main-loop safe) ====================== */
#define HEIGHT 20
#define WIDTH  40
...
static void snake_setup(void)
{
    ...
    fruitx = 10 + simple_rand(WIDTH - 20);
    fruity = 5  + simple_rand(HEIGHT - 10);
...
static void snake_draw(void)
{
	...
	*p++ = ' ';   /* extra space so footer doesn't glue to score */
    const char *footer = "   WASD=move, X=quit\r\n";
	...
	while (current_char < buffer_len) {
		...
		switch (toupper(cmd_buffer[current_char])) {
			...
			{	
				++current_char;
				if (current_char >= buffer_len) {
					usb_puts("ERROR: Missing Green LED phase!\r\n"); // Also missing 'N' for Snake
					return; // Don't parse the rest of the input
				}

				if (toupper(cmd_buffer[current_char]) == 'N') {  // quick & dirty "snake"
					in_snake_mode = 1;
					game_over_displayed = 0;
					g_serial_echo_enabled = 0;   /* silence echo during game */
					snake_setup();
					snake_need_redraw = 1;       /* draws on next main loop */
					usb_puts("\r\n=== SNAKE STARTED ===\r\n");
					return;
				}
